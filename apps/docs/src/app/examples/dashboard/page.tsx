...
const columns: TableColumn<Order>[] = [
  {
    ...
    header: "Order",
    cell: (row) => (
      <span className="font-mono text-xs text-[var(--velora-text-brand)]">{row.id}</span>
    ),
    width: 100,
  },
  {
  ...
  return (
    <Card variant="default" padding="md">
      <div className="flex items-start justify-between">
        <div>
          <p className="text-xs font-medium uppercase tracking-wider text-[var(--velora-text-tertiary)]">{title}</p>
          <p className="mt-1 text-2xl font-bold tabular-nums text-[var(--velora-text-primary)]">{value}</p>
          <div className={`mt-1 flex items-center gap-1 text-xs font-medium ${positive ? "text-[var(--velora-state-success)]" : "text-[var(--velora-state-danger)]"}`}>
            {positive ? <TrendingUp className="h-3 w-3" /> : <TrendingDown className="h-3 w-3" />}
            {change}
          </div>
        </div>
        <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[var(--velora-brand-subtle)]">
          <Icon className="h-5 w-5 text-[var(--velora-brand-default)]" />
        </div>
      </div>
      {progress !== undefined && (
        <div className="mt-3">
        ...
        <div className="flex flex-1 flex-col overflow-hidden">
          ...
          <main className="flex-1 overflow-y-auto p-6">
            <div className="mb-6 flex items-center justify-between">
              <div>
                <h1 className="text-xl font-semibold text-[var(--velora-text-primary)]">Dashboard</h1>
                <p className="text-sm text-[var(--velora-text-tertiary)]">Welcome back, Jane. Here&apos;s what&apos;s happening.</p>
              </div>
              <Button
                variant="gradient"
                size="sm"
                leadingIcon={<Plus className="h-4 w-4" />}
                onClick={() => success("Order created!", { description: "New order ORD-007 has been created." })}
              >
                New order
              </Button>
            </div>
              ...
              <CardHeader>
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <CardDescription>
                      {selectedRows.size > 0 ? `${selectedRows.size} selected` : "Showing 6 of 1,429 orders"}
                    </CardDescription>
                  </div>
                  {selectedRows.size > 0 && (
